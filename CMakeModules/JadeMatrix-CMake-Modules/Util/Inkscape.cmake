...
FUNCTION( ADD_INKSCAPE_IMAGE SVG_FILE OUTPUT_FILE )
    CMAKE_PARSE_ARGUMENTS(
        "ADD_INKSCAPE_IMAGE"
        "CREATE_INSTALL_RULE"
    ...
    IF( NOT DEFINED ADD_INKSCAPE_IMAGE_EXPORT )
        SET( ADD_INKSCAPE_IMAGE_EXPORT "page" )
    ENDIF()
    
    IF( ADD_INKSCAPE_IMAGE_EXPORT STREQUAL "page" )
        SET( EXPORT_ARG "--export-area-page" )
    ELSEIF( ADD_INKSCAPE_IMAGE_EXPORT STREQUAL "drawing" )
        SET( EXPORT_ARG "--export-area-drawing" )
    ELSE()
        SET( EXPORT_ARG "--export-area=${ADD_INKSCAPE_IMAGE_EXPORT}" )
    ENDIF()
    
    IF( NOT DEFINED ADD_INKSCAPE_IMAGE_DPI )
        SET( ADD_INKSCAPE_IMAGE_DPI "96" )
    ENDIF()
    
    IF( NOT DEFINED ADD_INKSCAPE_IMAGE_TARGET )
        STRING( REGEX REPLACE "[/ \t\n\r&<>\"']" "_" ADD_INKSCAPE_IMAGE_TARGET
            "${OUTPUT_FILE}"
        )
    ENDIF()
    ...
    # not given
    IF( NOT DEFINED ADD_INKSCAPE_IMAGE_OUTPUT_DIR )
        GET_FILENAME_COMPONENT(
            ADD_INKSCAPE_IMAGE_OUTPUT_DIR
            "${OUTPUT_FILE}"
            DIRECTORY
        )
    ENDIF()
    ...
    ADD_CUSTOM_COMMAND(
        ...
        COMMAND "${CMAKE_COMMAND}"
            -E make_directory "${CMAKE_CURRENT_BINARY_DIR}/${ADD_INKSCAPE_IMAGE_OUTPUT_DIR}"
        COMMAND Inkscape
            "--export-filename=${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT_FILE}"
            "--export-dpi=${ADD_INKSCAPE_IMAGE_DPI}"
            "${EXPORT_ARG}"
            "${CMAKE_CURRENT_SOURCE_DIR}/${SVG_FILE}"
        OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT_FILE}"
        MAIN_DEPENDENCY "${CMAKE_CURRENT_SOURCE_DIR}/${SVG_FILE}"
    )
    ADD_CUSTOM_TARGET( "${ADD_INKSCAPE_IMAGE_TARGET}"
        DEPENDS
            "${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT_FILE}"
            ${ADD_INKSCAPE_IMAGE_DEPENDS}
    )
    
    FOREACH( DEPENDEE IN LISTS ADD_INKSCAPE_IMAGE_DEPENDEES )
        ADD_DEPENDENCIES( "${DEPENDEE}" "${ADD_INKSCAPE_IMAGE_TARGET}" )
    ENDFOREACH()
    
    IF( ADD_INKSCAPE_IMAGE_CREATE_INSTALL_RULE )
        INSTALL(
            FILES "${CMAKE_CURRENT_BINARY_DIR}/${OUTPUT_FILE}"
            DESTINATION "${ADD_INKSCAPE_IMAGE_OUTPUT_DIR}"
        )
    ENDIF()
ENDFUNCTION()
